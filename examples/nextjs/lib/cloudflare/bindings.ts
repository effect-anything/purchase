...
interface Bindings {
  ...
  readonly getD1Databases: () => Effect.Effect<Array<D1Database>>

  readonly getKVNamespaces: () => Effect.Effect<Array<KVNamespace>>

  readonly getR2Buckets: () => Effect.Effect<Array<R2Bucket>>

  readonly getQueues: <Body = unknown>() => Effect.Effect<Array<Queue<Body>>>
...
const make = (env: Record<string, any>): Bindings => {
  ...
  const getBindingsByType = <T extends Binding>(type: BindingType): Array<T> => {
  ...
  return {
    ...
    getD1Databases: (): Effect.Effect<Array<D1Database>> => Effect.sync(() => getBindingsByType<D1Database>("d1")),

    getKVNamespaces: (): Effect.Effect<Array<KVNamespace>> => Effect.sync(() => getBindingsByType<KVNamespace>("kv")),

    getR2Buckets: (): Effect.Effect<Array<R2Bucket>> => Effect.sync(() => getBindingsByType<R2Bucket>("r2")),

    getQueues: <Body = unknown>(): Effect.Effect<Array<Queue<Body>>> =>
