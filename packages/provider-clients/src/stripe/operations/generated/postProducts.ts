...
export const PostProductsInput = Schema.Struct({
  ...
  default_price_data: Schema.optional(Schema.Struct({
  currency: Schema.String,
  currency_options: Schema.optional(Schema.Record({ key: Schema.String, value: Schema.Struct({
  custom_unit_amount: Schema.optional(Schema.Struct({
  ...
  marketing_features: Schema.optional(Schema.Array(Schema.Struct({
  name: Schema.String,
}))),
  package_dimensions: Schema.optional(Schema.Struct({
...
export const postProductsOperation = defineOperation({
  ...
  status: [200],
  contentType: "form",
  bodyParams: ["active", "default_price_data", "description", "expand", "id", "images", "marketing_features", "metadata", "name", "package_dimensions", "shippable", "statement_descriptor", "tax_code", "type", "unit_label", "url"]
})
