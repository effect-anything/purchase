...
export const CheckoutsUpdateInput = Schema.Struct({
  ...
  customer_metadata: Schema.optional(Schema.NullOr(Schema.Record({ key: Schema.String, value: Schema.Union(Schema.String, Schema.Number, Schema.Number, Schema.Boolean) }))),
...
export const checkoutsUpdateOperation = defineOperation({
  ...
  contentType: "json",
  pathParams: ["id"],
  bodyParams: ["custom_field_data", "product_id", "product_price_id", "amount", "seats", "is_business_customer", "customer_name", "customer_email", "customer_billing_name", "customer_billing_address", "customer_tax_id", "locale", "trial_interval", "trial_interval_count", "metadata", "currency", "discount_id", "allow_discount_codes", "require_billing_address", "allow_trial", "customer_ip_address", "customer_metadata", "success_url", "return_url", "embed_origin"]
})
