...
const globalContextMap = GlobalValue.globalValue("globalContextMap", () => {
  ...
  const log = {
    info: (_msg: string, _state?: ServiceState) => undefined,
  ...
  return {
    create: (
      identifier: string,
      tags: Array<Context.Tag<any, any>>,
      context: Context.Context<any>,
      close: Effect.Effect<void>
    ) =>
    ...
    acquire: <T extends Context.Tag<any, any>>(identifier: string, tags: Array<T>) =>
      Layer.scopedContext(
        Effect.gen(function* () {
    ...
    release: (identifier: string, tags: Array<Context.Tag<any, any>>) =>
      Effect.forEach(
        tags,
        (tag) => {
  ...
  pipe(
    Effect.gen(function* () {
      ...
      const [layers, tags] = res.reduce(
        ...
        [[], []] as [Array<Layer.Layer.Any>, Array<Context.Tag<any, any>>]
      )
...
type ExtractLayersService<T extends Array<Context.Tag<any, any>>> = {
  [K in keyof T]: Context.Tag.Identifier<NoInfer<T>[K]>
}
export const use = <T extends Array<Context.Tag<any, any>>>(identifier: string, ...tags: T) =>
