...
import { useAtomSet, useAtomSuspense, useAtomValue } from "../react.ts"
/**
 ...
 *
 * // In your component
 * const { value, loading, next, previous } = feed.useAtoms()
 * ```
 */
export const makeCachingPaginationAtom = <A, E>(
  ...
  const pullAtom = Atom.pull(handle({ perPage })).pipe(Atom.keepAlive)
  const loadingAtom = Atom.map(pullAtom, (pull) => pull.waiting).pipe(Atom.debounce(Duration.millis(50)))
  const doneAtom = Atom.map(pullAtom, (pull) => pull._tag === "Success" && pull.value.done)
  const hasNextPageAtom = Atom.make((get) => {
    const pullResult = get(pullAtom)
  ...
  const resultsAtom = Atom.make((ctx) =>
    Effect.gen(function* () {
      const pullResult = yield* ctx.result(pullAtom).pipe(Effect.optionFromOptional)
  ...
  const nextAtom = Atom.fn((_: void) =>
    Effect.gen(function* () {
      ...
      const pullResult = reg.get(pullAtom)
      ...
      reg.subscribe(pullAtom, (state) => {
        if (Result.isSuccess(state) && state.value.done) {
          return
        }

        if (!state.waiting) {
          Runtime.runSync(runtime)(Deferred.succeed(deferred, undefined))
        }
      })

      reg.set(pullAtom, undefined)
      const result = reg.get(pullAtom)
 ...
 *
 * // In your component
 * const { value, page, totalPages, goToPage } = table.useAtoms()
 * ```
 */
export const makeTablePaginationAtom = <A, E>(
  ...
  return {
    ...
    use: () => {
      ...
      const { value } = useAtomSuspense(pullAtom)
      const reset = useAtomSet(resetAtom, { mode: "promise" })
 ...
 *
 * // In your component
 * const { value, loading, loadMore } = feed.useAtoms()
 * ```
 */
export const makeContinuousLoadAtom = <A, E>(
  ...
  const pullAtom = Atom.pull(handle({ batchSize })).pipe(Atom.keepAlive)
  const loadingAtom = Atom.map(pullAtom, (pull) => pull.waiting).pipe(Atom.debounce(Duration.millis(50)))
  const doneAtom = Atom.map(pullAtom, (pull) => pull._tag === "Success" && pull.value.done)
  ...
  const loadMoreAtom = Atom.fn((_: void, ctx) => Effect.sync(() => ctx.set(pullAtom, undefined)))
  const resetAtom = Atom.fn((_: void, ctx) => Effect.sync(() => ctx.refresh(pullAtom)))
