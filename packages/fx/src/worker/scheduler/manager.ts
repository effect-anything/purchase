...
const make = Effect.gen(function* () {
  yield* EventPubSub.pipe(Effect.asVoid)
  ...
  const handle = <T extends CommandRequestHandle<any>>(commandHandler: T) =>
    Effect.sync(() => {
      commandHandle.set(commandHandler)
    })
  ...
  const on = <T extends EventRequestHandle<any>>(eventHandler: T) =>
    Effect.sync(() => {
      eventHandle.set(eventHandler)
    })
