...
import { table as writeDescription } from "./description.ts"
import { write as writeMermaid } from "./mermaid.ts"
import { tagValues, take } from "./utils.ts"
...
export const write = (schema: DMMF.Datamodel, config?: Record<string, string | string[] | undefined>): string => {
  ...
  const modelList: Array<DMMF.Model> = schema.models.filter((model) => !isHidden(model))
  ...
  for (const model of modelList) {
    const namespaces: Array<string> = takeTags("namespace")(model)
  ...
  // DESCRIPTIONS
  for (const model of modelList) {
    const describes: Array<string> = takeTags("describe")(model)
  ...
  // ERD ONLY
  for (const model of modelList) {
    const erdList: Array<string> = takeTags("erd")(model)
...
const takeTags =
  ...
  (model: DMMF.Model): Array<string> => [...new Set(tagValues(kind)(model).map((str) => str.split(" ")[0]))]
...
const findImplicits = (modelList: Array<DMMF.Model>) => {
  const dict: Map<string, DMMF.Model> = new Map()
  for (const model of modelList) {
    for (const field of model.fields) {
      if (field.kind !== "object" || field.isList !== true || field.isUnique !== false) continue

      const opposite: DMMF.Model | undefined = modelList.find((candidateModel) => candidateModel.name === field.type)
      const oppositeField = opposite?.fields.find(
        (candidateField) =>
          candidateField.kind === "object" && candidateField.isList && candidateField.type === model.name
      )
      if (opposite === undefined || oppositeField === undefined || model === opposite) continue

      const relations: Array<DMMF.Model> = [model, opposite].sort((x, y) => x.name.localeCompare(y.name))
...
const implicitToExplicit =
  ...
  (y: DMMF.Model): DMMF.Model => {
    ...
    const tagger = (kind: "namespace" | "describe" | "erd"): Array<string> =>
    ...
    ;(x.fields as Array<DMMF.Field>).push({
      kind: "object",
      name,
      type: name,
      isRequired: true,
    ...
    ;(y.fields as Array<DMMF.Field>).push({
      kind: "object",
      name,
      type: name,
      isRequired: true,
