...
export const write = (chapter: Array<DMMF.Model>) =>
  [
    "```mermaid",
    "erDiagram",
...
const writeField =
  ...
  (field: DMMF.Field): string =>
    [
      ...
      field.isId
        ? "PK"
        : model.fields.some((f) => f.kind === "object" && f.relationFromFields?.[0] === field.name)
          ? "FK"
          : field.isUnique
            ? "UK"
            : "", // constraint
...
const writeRelationship =
  (props: { group: Array<DMMF.Model>; model: DMMF.Model }) =>
  (field: DMMF.Field): string => {
    if (!field.relationFromFields?.length) return ""

    const column: string = field.relationFromFields[0]
    const scalar: DMMF.Field | undefined = props.model.fields.find((s) => column === s.dbName || column === s.name)
    if (scalar === undefined) return ""

    const target: DMMF.Model | undefined = props.group.find((t) => t.name === field.type)
    ...
    const arrow: string = [
      ...
      oneToOne &&
      props.group.some(
        (m) => m.name === field.type && m.fields.some((f) => f.relationName === field.relationName && !f.isRequired)
      )
        ? "o"
...
const isMandatoryMany = (props: { target: DMMF.Model; model: DMMF.Model; field: DMMF.Field }): boolean => {
  ...
  const values: Array<string> = tagValues("minItems")(opposite)
