...
export type ColumnConfig = {
  /**
   * 是否唯一。传入 string 时表示底层数据库约束名（Prisma `map`）
   */
  unique?: boolean | string
  /**
   * 是否为索引。传入 string 时表示底层数据库索引名（Prisma `map`）
   */
...
const JsonFromString = <A, I, R>(self: Schema.Schema<A, I, R>) =>
  Schema.parseJson(self).annotations({
    identifier: "Json",
    ...Option.match(AST.getDescriptionAnnotation(self.ast), {
      onNone: () => ({}),
      onSome: (description) => ({ description })
    })
  })

export const Json = <A, I, R>(self: Schema.Schema<A, I, R>) => self.pipe(JsonFromString)
...
export type ModelConfig = {
  ...
  primaryKey?: {
    fields: Array<string>
    /**
     * 底层数据库主键名（Prisma `map`）。SQLite provider 不支持。
     */
    map?: string
  }
}
...
export const NanoIdWithGenerate = <B extends string | symbol>(
  schema: Schema.brand<typeof Schema.String, B>,
  size?: number,
  factory: (size?: number) => string = (requestedSize) => nanoid.nanoid(requestedSize)
): VariantSchema.Overrideable<string & Brand.Brand<B>, string> =>
  VariantSchema.Overrideable(Schema.String, schema, {
    generate: Option.match({
