import { makeWorkflow, Workflow, type WorkflowInstance, Workflows } from "@effect-x/cloudflare/workflows"
import * as DurableObjectStorage from "../cloudflare/DurableObjectStorage.ts"
import { DestroyVault } from "../server/destroy-vault.ts"
import * as Storage from "../server/storage.ts"
import { getUserDurableId } from "../server/utils.ts"
...
const make = Effect.gen(function* () {
  ...
  const getInstance = Effect.fn("destroy-vault.getInstance")(function* ({
    ...
    const status = yield* Option.map(instance, (workflowInstance) => workflowInstance.status).pipe(
...
const WorkflowLive = DurableObjectStorage.Live
