...
import * as EventJournal from "@effect-x/event-log/EventJournal"
import * as EventLogRemote from "@effect-x/event-log/EventLogRemote"
import * as EventLogServer from "@effect-x/event-log/EventLogServer"
import { toUint8Array } from "@effect-x/event-log/Utils"
...
class DirectMessageDecodeError extends Data.TaggedError("DirectMessageDecodeError")<{
...
export abstract class EventLogDurableObject extends DurableObject {
  ...
  public handleDirectMessage(
    ...
    return Effect.try({
      try: () => EventLogRemote.decodeRequest(message),
      catch: (cause) => new DirectMessageDecodeError({ cause })
    }).pipe(
