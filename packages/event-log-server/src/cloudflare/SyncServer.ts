import type * as Vault from "../server/vault.ts"
...
import { DurableObjectError, type DurableObjectIdentity, DurableObjectIdentitySchema } from "./DurableObjectUtils.ts"
import * as EventLogSyncServer from "./EventLogSyncServer.ts"
import { type SyncServerInfo, type SyncStats, WorkerQueryParams } from "../schema.ts"
import { SyncHttpApi } from "../server/api.ts"
import { SyncVaultApiHttpLive } from "../server/http.ts"
import * as Ratelimit from "../server/ratelimit.ts"
import * as ServerSchema from "../server/schema.ts"
import * as Usage from "../server/usage.ts"
import * as User from "../server/user.ts"
import { SocketCloseCodes } from "@effect-x/event-log/EventLogConfig"
import { ScalarLayer } from "@effect-x/server/api/scalar"
...
const closeSocketResponse = (code: number, reason?: string | undefined) => {
      ...
      const state = !value
        ...
        : DurableState.decodeJSONEither(value).pipe(
            Either.match({
              ...
              onRight: (decodedValue) => decodedValue
            })
          )
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (request.method === "GET" && pathname === this.options.syncPath) {
      ...
      if (Either.isLeft(headersEither)) {
        return closeSocketResponse(SocketCloseCodes.UNAUTHORIZED, "Unauthorized")
      }
...
const toWebHandler = (
  ...
  layer: Layer.Layer<ServerSchema.UserSession | Vault.Vault, never, unknown>
) => {
  const DocsLive =
    // @ts-ignore
  ...
  const Live = pipe(DocsLive, Layer.provideMerge(ApiLive), withBaseLayer(env, context)) as Layer.Layer<any>

  return HttpApiBuilder.toWebHandler(Live, {
    middleware: Effect.fn(function* (app) {
      return yield* Effect.matchCauseEffect(app, {
        onSuccess: Effect.succeed,
        onFailure: (cause) => {
          const failure = Cause.failureOption(cause) as Option.Option<HttpServerError.RouteNotFound>
          if (Option.isSome(failure) && failure.value._tag === "RouteNotFound") {
...
export const makeDurableObject = (options: {
  layer: Layer.Layer<User.Authentication | Vault.Vault | Ratelimit.Ratelimit | Usage.Usage, never, unknown>
...
}): new (ctx: DurableObjectState, env: any) => SyncServerDurableObject => {
...
export const makeWorker = (options: {
  ...
  return {
    async fetch(request: Request, env: any, context: ExecutionContext): Promise<Response> {
      ...
      const Live = pipe(options.layer, withBaseLayer(env, context)) as Layer.Layer<
        User.Authentication | Vault.Vault | Ratelimit.Ratelimit | Usage.Usage
      >
      ...
      const program = Effect.gen(function* () {
        if (request.method === "GET" && pathname.startsWith(syncPath)) {
          ...
          if (Either.isLeft(paramsEither)) {
            return closeSocketResponse(SocketCloseCodes.MISSING_FIELDS, "Missing required fields")
          }
          ...
          if (!limitSuccess) {
            return closeSocketResponse(SocketCloseCodes.TOO_MANY_REQUESTS, "Too many requests")
          }
          ...
          const tier = Option.getOrThrow(Option.map(user, (currentUser) => currentUser.tier))
          const usage = yield* Usage.Usage

          const checkParams = {
            namespace,
            userId: Option.getOrThrow(Option.map(user, (currentUser) => currentUser.id)),
            publicKey
          }
          ...
          if (Either.isLeft(checkResult)) {
            return closeSocketResponse(checkResult.left.code, checkResult.left.message)
          }

          const identity = DurableObjectIdentitySchema.make({
            namespace,
            publicKey,
            userId: Option.map(user, (currentUser) => currentUser.id),
            userEmail: Option.map(user, (currentUser) => currentUser.email)
          })
        ...
        if (request.method === "POST" && pathname.startsWith(rpcPath)) {
          ...
          if (Either.isLeft(identityEither)) {
            const error = yield* HttpApiError.HttpApiDecodeError.fromParseError(identityEither.left)
      ...
      return program.pipe(Effect.provide(Live), (_) => Effect.runPromise(_, { signal: request.signal }))
    }
