...
import * as SqlDoProxy from "@effect-x/sql-do-proxy/SqlClient"
...
export const SyncStorageProxyConfig = Context.GenericTag<SyncStorageProxyConfig>(
  "@effect-x/event-log-server/RpcClient/SyncStorageProxyConfig"
)

export class SyncStorageProxyClient extends Context.Tag("@effect-x/event-log-server/RpcClient/SyncStorageProxyClient")<
  ...
  static Live = Layer.scoped(
    ...
    Effect.gen(function* () {
      ...
      const exec = Effect.fn(
        function* ({ sql, bindings }: { sql: string; bindings: Array<SqlStorageValue> }) {
