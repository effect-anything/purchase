...
import { SyncPublicKeyItem } from "../schema.ts"
import * as DestroyVault from "./destroy-vault.ts"
import { type Tier, Vault as VaultSchema } from "./schema.ts"
import * as Storage from "./storage.ts"
import { getUserUniqueId } from "./utils.ts"
...
const makeVault = Effect.gen(function* () {
  ...
  const vaultExists = flow(
    SqlSchema.findOne({
      ...
      execute: ({ userUniqueId, publicKey }) =>
        sql<{ count: number }>`
          SELECT EXISTS(
            SELECT 1 FROM "vault"
  ...
  const _findVaultByUser = SqlSchema.findOne({
    ...
    Result: VaultSchema,
    execute: ({ userUniqueId, publicKey }) =>
      sql<typeof VaultSchema.Encoded>`
  ...
  const findVaultsByUser = SqlSchema.findAll({
    ...
    Result: VaultSchema,
    execute: ({ userUniqueId }) =>
      sql<Array<typeof VaultSchema.Encoded>>`
   ...
   */
  const insertVault = (...args: Parameters<typeof VaultSchema.insert.make>) => insert(VaultSchema.insert.make(...args))
  ...
  const processPublicKeys = Effect.fn(function* ({
    ...
    const newItems: Array<(typeof items)[number]> = []
...
export class Vault extends Context.Tag("Vault")<Vault, Effect.Effect.Success<typeof makeVault>>() {}

export const VaultLive = Layer.effect(Vault, makeVault)
