import * as EventGroup from "@effect-x/event-log/EventGroup"
...
// - add more tools
// - task workflows ...
export const AgentEvents = Events.make(AgentEventGroup, {
  handlers: (handlers) =>
    Effect.gen(function* () {
      ...
      return handlers
        .handle("SetName", ({ payload }) => {
          return Effect.gen(function* () {
            ...
            yield* ClientEvents.trigger("Hi", { id: Math.round(Math.random() * 1000), message: `Hi ${payload.name}` })
          }).pipe(Effect.orDie)
        })
        .handle("Hi", ({ payload }) =>
          Effect.gen(function* () {
...
export const ClientEvents = Events.make(ClientEventGroup, {
  handlers: (handlers) =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient
      ...
      return handlers
        .handle("SetName", ({ payload }) => Effect.log("client trigger set name", payload.name))
        .handle("Hi", ({ payload }) =>
          Effect.gen(function* () {
