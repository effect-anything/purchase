import { CloudflareBindings } from "@effect-x/cloudflare/bindings"
import * as DurableObjectStorage from "../../../src/cloudflare/DurableObjectStorage.ts"
import * as SyncServer from "../../../src/cloudflare/SyncServer.ts"
import * as Ratelimit from "../../../src/server/ratelimit.ts"
import { UserSession } from "../../../src/server/schema.ts"
import * as Usage from "../../../src/server/usage.ts"
import { Authentication, Tier } from "../../../src/server/user.ts"
import * as Vault from "../../../src/server/vault.ts"
import { DestroyVaultWorkflow, Live as DestroyVaultWorkflowLive } from "../../../src/workflows/destroy-vault.ts"
...
const AuthenticationLive = Layer.sync(Authentication, () => {
  return Authentication.of({
    fromToken: ({ namespace: _namespace, token }) =>
      Effect.gen(function* () {
        void _namespace
...
const RatelimitLive = Layer.effect(
  ...
  Effect.gen(function* () {
    const { ratelimitBasic, ratelimitLow } = yield* CloudflareBindings.use((_) =>
      ...
      Effect.flatMap(
        Option.match({
          onNone: () => Effect.dieMessage("Ratelimit tier not found, basic, low"),
          onSome: Effect.succeed
        })
      )
    )
    ...
    const limit = Effect.fnUntraced(function* ({ key }: { key: string }) {
      ...
      const limiter = ratelimitBasic

      return yield* Effect.promise(() => limiter.limit({ key }).then((_) => _.success))
    })
...
const VaultLayer = Vault.VaultLive.pipe(Layer.provide([DurableObjectStorage.Live, DestroyVaultWorkflowLive, DBLive]))

const UsageLive = Usage.Live.pipe(Layer.provide(VaultLayer))

const Live: Layer.Layer<Authentication | Ratelimit.Ratelimit | Usage.Usage | Vault.Vault, never, unknown> =
  Layer.mergeAll(AuthenticationLive, RatelimitLive, UsageLive, VaultLayer)
