...
import { BadRequestError, InternalServerError } from "./server.ts"

import type { StandardError } from "./domains.ts"
...
export function encode(cause: any): StandardError {
  ...
  const renderCause = (source: any): any => {
    if (!isProduction && source && (source.message || source.stack)) {
      return {
  ...
  const renderStack = (error: any, effectCause: any) => {
    return isProduction
      ? undefined
      : (Cause.isCause(effectCause) && Cause.pretty(effectCause, { renderErrorCause: true })) || error.stack
  }

  const formatError = (error: unknown, effectCause?: Cause.Cause<any> | undefined): StandardError => {
    if (isTaggedError(error)) {
      if (isParseError(error)) {
        return {
          _tag: BadRequestError._tag,
          message: error.message || "",
          stack: renderStack(error, effectCause),
    ...
    return {
      _tag: InternalServerError._tag,
      message: err.message || "",
      stack: renderStack(err, effectCause),
