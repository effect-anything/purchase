...
import { findCookieByName } from "./cookie.ts"
import {
  type Location,
  matchPath,
  type Params,
...
export { useLanguageChangeRevalidator } from "./hooks/use-react-router-utils.ts"
export { useActionData, useFetcherData, useLoaderData, useRouteLoaderData } from "./hooks/use-safe-response.ts"
...
export function isAppRoute(pattern: Array<string>, pathname: string) {
  return pattern.some((routePattern) => matchPath(routePattern, pathname))
}
