import type { RatelimitError } from "@effect-x/errors/server"
import type { RatelimiterResponse, RatelimitOptions, RatelimitUserOptions } from "@effect-x/server/ratelimit/make"
...
import * as ErrorEncoder from "@effect-x/errors/encoder"
import { I18n } from "@effect-x/i18n/i18n"
import { makeI18nService } from "@effect-x/i18n/i18n.server"
import {
...
import { transformReactRouterError } from "./errors/server.ts"
import { Cookies, makeRequestContext, ReactRouterRedirect, RequestContext } from "./request.ts"
import { type ReactRouterData, ReactRouterResult } from "./response.ts"
import { Ratelimiter } from "@effect-x/server/ratelimit/make"
...
const shouldSkipTracing = (request: Request) => {
  ...
  const skipList: Array<string> = ["Uptime-Kuma"]
...
const createRequestHandler =
  ...
  <A, E, R, E1>(
    ...
    const loadContext = args.context as RequestLoadContext<E, R>
    let main = body
    ...
    const program = pipe(
      ...
      (effect) =>
        loadContext.i18n
          ? (Effect.provideService(effect, I18n, makeI18nService(loadContext.i18n)) as typeof effect)
          : effect,
      ...
      Effect.catchAllCause((cause_) => {
        ...
        if (cause._tag === "Die") {
          if (cause.defect && isReactRouterRedirect(cause.defect)) {
            const redirect = cause.defect as ReactRouterRedirect
            const responseHeaders = new Headers(redirect.response.headers)
            ...
            return Effect.succeed(
              new ReactRouterRedirect({
                response: new Response(redirect.response.body, {
                  status: redirect.response.status,
                  statusText: redirect.response.statusText,
...
const make =
  ...
  <A, E, R>(body: Effect.Effect<A, E, R>, config?: RequestHandlerConfig | undefined) => {
    ...
    const json_ = (args: LoaderFunctionArgs): Promise<ReactRouterData<A, FormatInternalError<E>>> => {
      ...
      // During SPA prerender (build time), context.runtime is undefined
      // Check for X-React-Router-SPA-Mode header to detect prerender
      const isSpaPrerender = args.request.headers.get("X-React-Router-SPA-Mode") === "yes"

      if (!context.runtime) {
        if (isSpaPrerender) {
          // Return a minimal response for SPA prerender
          // The actual data will be loaded client-side
          const data_: ReactRouterData<A, FormatInternalError<E>> = {
            success: true,
            result: {} as A
          }
          return Promise.resolve(data_)
        }
        throw new Error("Runtime context is not available. This loader requires a server runtime.")
      }
    ...
    const runSchema = <A2>(schema: Schema.Schema<A, A2>, args: LoaderFunctionArgs) => {
      ...
      const handled = handler<A, E, R, FormatInternalError<E>>(args, body, (result) => {
        if (result._tag === "ReactRouterResultFailure") {
      ...
      return handled as Effect.Effect<Exit.Exit<ReactRouterResult<A2, FormatInternalError<E>>>>
    }

    const schema = <A2>(resultSchema: Schema.Schema<A, A2>) => {
      const fn = (args: LoaderFunctionArgs): Promise<ReactRouterData<A2, FormatInternalError<E>>> => {
        const context = args.context as unknown as RequestLoadContext<E, R>

        return context.runtime
          .runPromise(runSchema(resultSchema, args), {
      ...
      Object.defineProperty(fn, "effect", {
        enumerable: false,
        configurable: false,
        get: () => (args: LoaderFunctionArgs) => runSchema(resultSchema, args).pipe(Effect.map(toResponse))
      })
