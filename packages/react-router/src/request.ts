...
const withRequestContext = <E, A>(effect: (requestContext: RequestContext) => Effect.Effect<A, E>) =>
  Effect.context<never>().pipe(
    Effect.map((contextValue) => Context.get(contextValue as Context.Context<RequestContext>, RequestContext)),
    Effect.flatMap(effect)
  )

export const request = withRequestContext((requestContext) => Effect.succeed(requestContext.request))

export const context = withRequestContext((requestContext) => Effect.succeed(requestContext.context))

export const getFormDataEntries = withRequestContext(
  (requestContext): Effect.Effect<Record<string, any>, ReactRouterFormDataParseError, never> =>
    Effect.tryPromise({
      try: () => requestContext.request.formData(),
      catch: (e: unknown) =>
...
export const getBody = <A, I>(schema: Schema.Schema<A, I>): Effect.Effect<A, ReactRouterBodyParseError, never> =>
  withRequestContext((requestContext) =>
    Effect.tryPromise({
      try: () => requestContext.request.json(),
      catch: (e: unknown) =>
...
export const getSearchParams = <A, I>(
  ...
  withRequestContext((requestContext) =>
    Effect.try({
      try: () => Object.fromEntries(new URL(requestContext.request.url).searchParams.entries()),
      catch: (error: unknown) =>
...
export const getParams = <I, A>(schema: Schema.Schema<A, I>): Effect.Effect<A, ReactRouterParamsParseError, never> =>
  withRequestContext((requestContext) =>
    Schema.decodeUnknown(schema)(requestContext.params).pipe(
...
class Cookies_ {
  private get records() {
    return withRequestContext((requestContext) => Effect.sync(() => requestContext.stub.cookies.records)).pipe(
      Effect.cached,
      Effect.flatten
    )
  }
  ...
  private get mutable() {
    return withRequestContext((requestContext) => Effect.sync(() => requestContext.stub.cookies.mutable)).pipe(
      Effect.cached,
      Effect.flatten
    )
  }
  ...
  get fromHeader() {
    return withRequestContext((requestContext) => Effect.sync(() => requestContext.stub.cookies.raw)).pipe(
      Effect.cached,
      Effect.flatten,
...
class Headers_ {
  private static get mutable() {
    return withRequestContext((requestContext) => Effect.sync(() => requestContext.stub.headers)).pipe(
      Effect.cached,
      Effect.flatten
    )
  }
  ...
  append: {
    ...
    (...args: Array<[string, string]>): Effect.Effect<void>
  } = (...args) =>
  ...
  get keys(): Effect.Effect<Array<string>> {
    return Headers_.mutable.pipe(
  ...
  get values(): Effect.Effect<Array<string>> {
    return Headers_.mutable.pipe(
  ...
  get entries(): Effect.Effect<Array<[string, string]>> {
    return Headers_.mutable.pipe(
