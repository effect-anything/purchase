...
import { parseConfig } from "./runtime.ts"
import getPorts from "@effect-x/server/port"
...
import type { RolldownOutput, BuildOptions } from "rolldown"
...
const workflowInstanceKey = (binding: string, id: string) => `${binding}:${id}`
...
const WORKFLOW_POLL_INTERVAL_MS = 50
const WORKFLOW_WAIT_TIMEOUT_MS = 30_000
const WORKFLOW_FINITE_STATUSES = new Set<Workflows.InstanceStatus["status"]>(["complete", "errored", "terminated"])
const WORKFLOW_PROXY_GET_RETRY_MS = 50
const WORKFLOW_PROXY_GET_RETRY_ATTEMPTS = 200

type LocalDevWorkflowStatus = Workflows.InstanceStatus & {
  __LOCAL_DEV_STEP_OUTPUTS?: Array<unknown>
}

const sleep = (ms: number) => new Promise<void>((resolve) => setTimeout(resolve, ms))

const decodeLocalDevStepOutput = (output: unknown) => {
  if (!output || typeof output !== "object" || !("_tag" in output)) {
    return output
  }

  const exit = output as { _tag?: string; value?: unknown }
  return exit._tag === "Success" ? exit.value : output
}

const getWorkflowInstanceWithRetry = async (workflowBinding: Workflow, instanceId: string) => {
  let lastError: unknown

  for (let attempt = 0; attempt < WORKFLOW_PROXY_GET_RETRY_ATTEMPTS; attempt++) {
    try {
      return await workflowBinding.get(instanceId)
    } catch (error) {
      lastError = error
      ...
      await sleep(WORKFLOW_PROXY_GET_RETRY_MS)
    }
  }
...
class WorkflowIntrospectorInstance {
  ...
  private stepIndexes = new Map<string, number>()
  ...
  private async getStatusSnapshot() {
    ...
    return (await ins.status()) as unknown as LocalDevWorkflowStatus
  }

  private getStepIndex(name: string, index?: number | undefined) {
    if (index !== undefined) {
      return index
    }

    const existing = this.stepIndexes.get(name)
    if (existing !== undefined) {
      return existing
    }

    const nextIndex = this.stepIndexes.size
    this.stepIndexes.set(name, nextIndex)
    return nextIndex
  }

  waitForStepResult(name: string, index?: number | undefined) {
    return Effect.promise(async () => {
      const stepIndex = this.getStepIndex(name, index)
      const timeoutAt = Date.now() + WORKFLOW_WAIT_TIMEOUT_MS
      let status: LocalDevWorkflowStatus | undefined

      while (Date.now() < timeoutAt) {
        status = await this.getStatusSnapshot()
        const outputs = status.__LOCAL_DEV_STEP_OUTPUTS ?? []

        if (stepIndex < outputs.length) {
          return decodeLocalDevStepOutput(outputs[stepIndex])
        }

        if (WORKFLOW_FINITE_STATUSES.has(status.status)) {
          break
        }
      ...
      const lastStatus = status?.status ?? "unknown"
      const suffix = status?.error ? ` Error: ${status.error}` : ""
      throw new Error(
        `[WorkflowIntrospector] Timed out waiting for step '${name}' (index ${stepIndex}) on instance ${this.id}. Last status was '${lastStatus}'.${suffix}`
      )
    })
  }
  ...
  waitForStatus(status: Workflows.InstanceStatus["status"]) {
    return Effect.promise(async () => {
      const timeoutAt = Date.now() + WORKFLOW_WAIT_TIMEOUT_MS
      let current: LocalDevWorkflowStatus | undefined

      while (Date.now() < timeoutAt) {
        current = await this.getStatusSnapshot()

        if (current.status === status) {
          return
        }

        if (WORKFLOW_FINITE_STATUSES.has(current.status)) {
          break
        }
      ...
      const lastStatus = current?.status ?? "unknown"
      const suffix = current?.error ? ` Error: ${current.error}` : ""
      throw new Error(
        `[WorkflowIntrospector] Timed out waiting for status '${status}' on instance ${this.id}. Last status was '${lastStatus}'.${suffix}`
      )
    })
  }

  status() {
    return Effect.promise(async () => await this.getStatusSnapshot())
  }
 ...
 */
async function waitForWaitUntil(/* mut */ waitUntil: Array<unknown>): Promise<void> {
...
type MiniflareOperations = {
  ...
  readonly workflows: {
    ...
    readonly getAll: () => Effect.Effect<Array<WorkflowIntrospectorInstance>>
...
const make = (
  ...
  Effect.gen(function* () {
    const port = options.port ?? (yield* Effect.tryPromise(() => getPorts({ port: 0 })).pipe(Effect.repeatN(3)))
    const inspectorPort = yield* Effect.tryPromise(() => getPorts({ port: 0, exclude: [port] })).pipe(Effect.repeatN(3))
    ...
    const bundler = yield* Effect.tryPromise(() => import("rolldown")).pipe(Effect.orDie)
    const build = bundler.build as (options: BuildOptions) => Promise<RolldownOutput>

    yield* Effect.forEach(
      workersWranglerConfigs,
      Effect.fn(function* (config) {
        const { wranglerConfig } = config
        const workersName = wranglerConfig.name ?? path.basename(path.dirname(config.path))
        ...
        if (config.bundle) {
          ...
          const files = results.output.map((file: (typeof results.output)[number]) => {
            const filePath = file.fileName ?? `${workersName}.${file.type === "chunk" ? "js" : "asset"}`
            const filename = path.basename(filePath)
            const isMain = file.type === "chunk"
            return {
              path: path.join(workersCompiledPath, workersName, filePath),
              filename,
              main: isMain,
    ...
    const globalWaitUntil: Array<unknown> = []
    ...
    const workflowInstances = new Map<string, Awaited<ReturnType<Workflow["get"]>>>()
    ...
    const rememberWorkflowInstance = (binding: string, instance: Awaited<ReturnType<Workflow["get"]>>) => {
      workflowInstances.set(workflowInstanceKey(binding, instance.id), instance)
      return instance
    }
    const resolveWorkflowInstance = async (binding: string, workflowBinding: Workflow, instanceId: string) => {
      const cached = workflowInstances.get(workflowInstanceKey(binding, instanceId))
      if (cached) {
        return cached
      }

      return rememberWorkflowInstance(binding, await getWorkflowInstanceWithRetry(workflowBinding, instanceId))
    }

    const runWorkflowModifierCallback = async () => {
      if (modifierCallbacks.length === 0) {
        return
      }
    ...
    const workflows = {
      ...
      dispose: Effect.fnUntraced(function* () {
        ...
        for (const item of workflowInstanceIntrospectors) {
          ...
          yield* Effect.promise(async () => binding.unsafeAbort(item.id, "Instance dispose"))
        }
      }),
    ...
    const wranglerWorkers: Array<WorkerOptions> = workersWranglerConfigs
      .map(({ wranglerConfig: config, path: configPath }) => ({
        config,
        configPath
      }))
      .map(({ config, configPath }) => {
        const workerName = config.name ?? path.basename(path.dirname(configPath))
        const mainScript = (workersFiles.get(workerName) ?? []).find((script) => !!script.main)
            ...
            ? {
                async DEV_WORKFLOW_PROXY(request: Request) {
                  ...
                  if (url.pathname === "/workflow") {
                    ...
                    try {
                      if (method === "get") {
                        const result = await resolveWorkflowInstance(binding, workflowBinding, args[0])
                      ...
                      if (method === "create") {
                        ...
                        const id = args[0].id
                        const result = rememberWorkflowInstance(binding, await workflowBinding.create(args[0]))
                      ...
                      if (method === "createBatch") {
                        const result = (await workflowBinding.createBatch(args[0])).map((instance) =>
                          rememberWorkflowInstance(binding, instance)
                        )
                  ...
                  if (url.pathname === "/instance") {
                    ...
                    const instance = await resolveWorkflowInstance(binding, workflowBinding, instanceId)
                    if (!instance) {
                    ...
                    switch (method) {
                      ...
                      case "sendEvent":
                        try {
                          await instance.sendEvent(args[0])
                        } catch (error: any) {
                          result = { error: error.message }
                        }
                        break
    ...
    const workersOptions: Array<WorkerOptions> = wranglerWorkers.concat(configWorkers)
...
function generateDummyScript(config: Unstable_Config): string {
  ...
  const workflowsClasses: Array<string> = []
...
export const MiniflareLive = Layer.unwrapScoped(
  Effect.gen(function* () {
    ...
    return Layer.mergeAll(KV.Default, R2.Default, RatelimiterLocal).pipe(
      Layer.provideMerge(
        Layer.mergeAll(
