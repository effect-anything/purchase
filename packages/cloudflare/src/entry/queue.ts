...
import { MessageStatusCount, QueueEvent, type QueueEventMessage, type QueueProcessResult } from "../queue.ts"
...
>() {
  static async run(
    ...
    const program = Effect.gen(function* () {
      let messages = batch.messages as Array<Message<any>>
      ...
      const process: {
        ...
        const { successes, failures } = yield* Effect.reduce(
          ...
          {
            successes: [] as Array<Msg>,
            failures: [] as Array<Msg>
          },
          (acc, message) => {
