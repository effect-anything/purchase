...
const fetch: HttpClient.HttpClient = HttpClient.make((request, url, signal, fiber) => {
  ...
  const nativeFetch: typeof globalThis.fetch = context.unsafeMap.get(FetchHttpClient.Fetch.key) ?? globalThis.fetch
    ...
    Layer.provide(
      Layer.effect(
        ...
        pipe(
          ...
          Effect.map((fetcher) => fetcher.fetch.bind(fetcher) as typeof globalThis.fetch)
        )
      )
    ),
    Layer.map((_) => {
        ...
        flow(
          ...
          HttpClient.transformResponse((responseEffect) =>
            options?.transformResponse
              ? (Effect.flatMap(responseEffect, options.transformResponse) as typeof responseEffect)
              : responseEffect
          ),
