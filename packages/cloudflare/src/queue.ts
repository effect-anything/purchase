import type { Message, Queue as CFQueue, QueueRetryOptions } from "@cloudflare/workers-types"
...
export type QueueProcessResult<TSuccess, TFailure = TSuccess> = {
  readonly successes: Array<QueueEventMessage<TSuccess>>
  readonly failures: Array<QueueEventMessage<TFailure>>
}

export interface QueueEvent {
  readonly queue: string
  readonly messages: Effect.Effect<Array<Message<unknown>>>
  process: {
    <A = unknown, E = any>(
      effect: (message: QueueEventMessage<A>) => Effect.Effect<void, E, never>
    ): Effect.Effect<QueueProcessResult<A>, never>
    <A = unknown, I = unknown, E = any>(
      schema: Schema.Schema<A, I>,
      effect: (message: QueueEventMessage<A>) => Effect.Effect<void, E, never>
    ): Effect.Effect<QueueProcessResult<A, unknown>, never>
  }
  ackAll: Effect.Effect<void>
  retryAll: (options?: QueueRetryOptions) => Effect.Effect<void>
}
