...
import { DecryptDEKError, DecryptionError, type EncryptedDEKError, EncryptionError } from "./Error.ts"
 ...
 * @category encryption
 */
export class EventLogEncryption extends Context.Tag("@effect-x/event-log/EventLogEncryption")<
  EventLogEncryption,
  {
    /**
...
export const make: Effect.Effect<typeof EventLogEncryption.Service, never, Crypto.Crypto> = Effect.gen(function* () {
   ...
   * 3. 详细的日志记录
   */
  const getOrCreateActiveDEK = Effect.gen(function* () {
    ...
    pipe(
      Effect.gen(function* () {
        ...
        return yield* EventJournal.Entry.decodeArray(decryptedEntries).pipe(
          Effect.map((decodedEntries) =>
            decodedEntries.map(
              (decodedEntry, i) =>
                new EventJournal.RemoteEntry({ remoteSequence: entries[i].sequence, entry: decodedEntry })
            )
          )
        )
      }),
