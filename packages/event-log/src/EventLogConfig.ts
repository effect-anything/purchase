...
export const EventLogConfig = Config.all({
  ...
  dekSalt: Config.string("DEK_SALT").pipe(
    Config.nested("SYNC"),
    Config.orElse(() => Config.succeed("@effect-x/dek-generation"))
  ),

  dekCacheSalt: Config.string("DEK_CACHE_SALT").pipe(
    Config.nested("SYNC"),
    Config.orElse(() => Config.succeed("@effect-x/dek-cache"))
  ),

  masterKeySalt: Config.string("MASTER_KEY_SALT").pipe(
    Config.nested("SYNC"),
    Config.orElse(() => Config.succeed("@effect-x/master-key"))
  ),
  ...
  ecdhSalt: Config.string("ECDH_SALT").pipe(
    Config.nested("SYNC"),
    Config.orElse(() => Config.succeed("@effect-x/ecdh-encryption"))
  )
})
 ...
 */
export const isErrorCode: Array<number> = [
  1005, // 客户端手动正常关闭
