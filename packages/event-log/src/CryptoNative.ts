...
import { Crypto, EncryptedDEK } from "./Crypto.ts"
...
type QuickCryptoApi = {
  readonly createHmac: (
    ...
    readonly update: (data: Buffer) => void
    readonly digest: () => Buffer
  }
  readonly createCipheriv: (
    algorithm: "aes-256-gcm",
    key: Buffer | QuickCryptoKey,
    iv: Buffer | Uint8Array
  ) => {
    readonly update: (data: Buffer | Uint8Array) => Buffer
    readonly final: () => Buffer
    readonly getAuthTag: () => Buffer
  }
  readonly createDecipheriv: (
    ...
    readonly update: (data: Buffer | Uint8Array) => Buffer
    readonly final: () => Buffer
    readonly setAuthTag: (tag: Buffer) => void
  }
}

const quickCrypto = QuickCrypto as unknown as QuickCryptoApi
