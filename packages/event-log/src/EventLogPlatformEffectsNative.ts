import type * as OPSqlClient from "@effect-x/sql-op-sqlite/SqlClient"
...
export interface ExternalDatabaseStorage {
  readonly get: Array<Effect.Effect<number, never, never>>
}

export const ExternalDatabaseStorage = Context.GenericTag<ExternalDatabaseStorage>(
  "@effect-x/event-log/EventLogPlatformEffects/ExternalDatabaseStorage"
)

export const Live = Layer.effect(
  EventLogPlatformEffects.EventLogPlatformEffects,
  Effect.gen(function* () {
    const sql = (yield* SqlClient.SqlClient) as unknown as OPSqlClient.SqliteClient
