import type { StoragePublicKeyUpdateItem } from "./Identity.ts"
import type * as HttpClientRequest from "@effect/platform/HttpClientRequest"

import * as Bip39 from "./Bip39.ts"
import * as Crypto from "./Crypto.ts"
import { InvalidMnemonicError } from "./Error.ts"
import * as EventLogConfig from "./EventLogConfig.ts"
import * as Identity_ from "./Identity.ts"
import * as IdentityStorage from "./IdentityStorage.ts"
...
const make = Effect.gen(function* () {
  ...
  const getStorageMnemonic = Effect.gen(function* () {
  ...
  const syncPublicKey = Effect.fn(`${SPAN_ID}.syncPublicKey`)(function* (targetPublicKey: string) {
    const query = btoa(`${config.namespace}:${targetPublicKey}`)
  ...
  const deletePublicKey = Effect.fn(`${SPAN_ID}.deletePublicKey`)(function* (targetPublicKey: string) {
    const query = btoa(`${config.namespace}:${targetPublicKey}`)
    const url = `${config.syncUrl}/api/destroy`

    yield* Effect.logTrace("Deleting public key", { publicKey: targetPublicKey })

    const publicKeys = yield* storage.getAllPublicKeys()
    const deleted = publicKeys.find((_) => _.publicKey === targetPublicKey)

    if (!deleted) {
      yield* Effect.logTrace("Public key not found", { publicKey: targetPublicKey })
      return
    }

    yield* storage.deletePublicKey(targetPublicKey)
  ...
  const upsertPublicKey = Effect.fn(`${SPAN_ID}.upsertPublicKey`)(function* (
    ...
    yield* Effect.logTrace("Updating public key locally").pipe(
      Effect.annotateLogs({ publicKey: targetPublicKey, data })
    )

    yield* storage.upsertPublicKey(targetPublicKey, data).pipe(Effect.zipRight(invalidatePublicKeys))
  })
  ...
  const updatePublicKey = Effect.fn(`${SPAN_ID}.updatePublicKey`)(function* (
    ...
    yield* Effect.logTrace("Updating public key ").pipe(Effect.annotateLogs({ publicKey: targetPublicKey, data }))

    yield* Effect.all(
      [
        upsertPublicKey(targetPublicKey, data),
        client
          .patch(url, {
            urlParams: { q: query },
