...
class CryptoDigestError extends Data.TaggedError("CryptoDigestError")<{
  ...
  Effect.gen(function* () {
    ...
    const sha256_ = Effect.fn(function* (data: Uint8Array<ArrayBufferLike> | string) {
      ...
      const result = yield* Effect.tryPromise({
        ...
        catch: (cause) => new CryptoDigestError({ algorithm: "SHA-256", cause })
      })
    ...
    const decryptDEK = Effect.fn(function* (
      ...
      const key = yield* Effect.promise(() =>
        crypto.subtle.importKey("raw", encryptionKeyBytes.slice(), { name: "AES-GCM", length: 256 }, false, ["decrypt"])
      )
