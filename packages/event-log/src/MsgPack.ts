...
export const packSchema =
  <A, I, R>(schemaDef: Schema.Schema<A, I, R>) =>
  <IE = never, Done = unknown>(): Channel.Channel<
    ...
    R
  > =>
    Channel.pipeTo(ChannelSchema.encode(schemaDef)(), pack())
...
> =>
  Channel.flatMap(
    ...
    (packr) => {
      let incomplete: Uint8Array<ArrayBufferLike> | undefined
      const unpackChunk = (value: Chunk.Chunk<Uint8Array>) =>
        Effect.try({
          try: () =>
      ...
      > = Channel.readWithCause({
        onInput: (input: Chunk.Chunk<Uint8Array>) =>
          Channel.zipRight(Channel.flatMap(unpackChunk(input), Channel.write), loop),
        onFailure: (cause) => Channel.failCause(cause),
        onDone: Channel.succeed
      })
...
export const unpackSchema =
  ...
  <IE = never, Done = unknown>(): Channel.Channel<
    ...
    R
  > =>
    Channel.pipeTo(unpack(), ChannelSchema.decodeUnknown(schemaDef)())
...
export const duplex = <R, IE, OE, OutDone, InDone>(
  ...
  Channel.pipeTo(Channel.pipeTo(pack(), self), unpack())
 ...
 * @category schemas
 */
export const schema = <S extends Schema.Schema.Any>(schemaDef: S): schema<S> =>
  Schema.transformOrFail(Schema.Uint8ArrayFromSelf, schemaDef, {
    decode(fromA, _, ast) {
