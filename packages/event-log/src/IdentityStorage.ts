import type * as Tag from "./Identity.ts"
...
export const IdentityStorage = Context.GenericTag<IdentityStorage>("@effect-x/event-log/IdentityStorage")
...
const addRequestSuccessListener = (request: IDBRequest, listener: (event: Event) => void) => {
  request.addEventListener("success", listener, { once: true })
}

const addRequestErrorListener = (request: IDBRequest, listener: (event: Event) => void) => {
  request.addEventListener("error", listener, { once: true })
}

const addTransactionCompleteListener = (transaction: IDBTransaction, listener: (event: Event) => void) => {
  transaction.addEventListener("complete", listener, { once: true })
}

const addTransactionErrorListener = (transaction: IDBTransaction, listener: (event: Event) => void) => {
  ...
  const makeIndexedDB = Effect.promise(
    () =>
      new Promise<IDBDatabase>((resolve, reject) => {
        ...
        addRequestErrorListener(request, () => reject(request.error))
        addRequestSuccessListener(request, () => resolve(request.result))
          ...
          Effect.promise(
            () =>
              new Promise<void>((resolve, reject) => {
                ...
                addTransactionCompleteListener(tx, () => resolve())
                addTransactionErrorListener(tx, () => reject(tx.error))
              })
          )
        ),
        Effect.scoped
          ...
          Effect.promise(
            () =>
              new Promise<void>((resolve, reject) => {
                ...
                addRequestSuccessListener(getRequest, () => {
                  const now = new Date()
                ...
                addRequestErrorListener(getRequest, () => reject(getRequest.error))
              })
          )
        ),
        Effect.scoped
          ...
          Effect.promise(
            () =>
              new Promise<ReadonlyArray<EventLogSchema.StoragePublicKeyItem>>((resolve, reject) => {
                ...
                const items: Array<typeof EventLogSchema.StoragePublicKeyItem.Encoded> = []

                addRequestSuccessListener(request, (event) => {
