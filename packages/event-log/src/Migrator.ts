...
const formatMigrationName = (filename: string) => {
  ...
  const [matchPart, part1, part2] = match.filter((matchSegment) => {
    return matchSegment !== "-" && matchSegment !== "_"
  })

  if (!part2 || !part1 || !matchPart) {
    throw new Error(`Invalid migration filename: ${filename}`)
  }
    ...
    Layer.provide(
      Layer.unwrapEffect(
        Effect.gen(function* () {
          ...
          const sqls: Array<ResolvedMigration> = Object.entries(migrations)
            .map(([path, sql]) => {
...
const make = Effect.gen(function* () {
  ...
  const start = Effect.gen(function* () {
    ...
    const ensureMigrationsTableExist = pipe(
      Effect.logTrace("Ensured migration table"),
      Effect.zipRight(
        sql`
    ...
    const install = Effect.logTrace("Install schema").pipe(
    ...
    const migrate = Effect.gen(function* () {
      if (migrationSqls.length === 0) {
        yield* Effect.logTrace("No migrations to apply")
        return
      }

      yield* Effect.logTrace("Migrate")
      ...
      const successMigrations: Array<Migration> = []
      const failedMigrations: Array<Migration> = []
      ...
      yield* Effect.forEach(
        ...
        Effect.tap(() =>
          Effect.logTrace("Done running migrations").pipe(
            Effect.annotateLogs(
              "failed_migrations",
      ...
      if (successMigrations.length > 0) {
        yield* Effect.logTrace("Save migrations").pipe(
      ...
      yield* latestMigration.pipe(
        Effect.flatMap(
          Option.match({
            onNone: () => Effect.logTrace("Migrations complete"),
            onSome: (_) =>
              pipe(Effect.logTrace("Migrations complete"), Effect.annotateLogs("latest_migration_name", _.name))
          })
        ),
        Effect.annotateLogs({
    ...
    const run: Effect.Effect<void, MigrationError, never> = sql`SELECT * from ${sql(
      migrationsTable
    )} where name='schema-sql'`.pipe(
      Effect.mapError(toMigrationError("Migration failed")),
      Effect.flatMap((rows) => {
        const next: Effect.Effect<void | undefined, SqlError | MigrationError, never> =
          rows.length === 0 ? (String.isNonEmpty(schemaSql) ? install : migrate) : migrate

        return next.pipe(Effect.asVoid, Effect.mapError(toMigrationError("Migration failed")))
      })
    )
