...
import { makeContextService } from "@effect-x/fx/context"
 ...
 * 配置选项
 */
export interface MakeEventLogStateOptions {
  /**
   * 数据库表名
   */
...
/**
 * 创建基于 SQL 的 EventLog State
 *
 * @example
 * ```ts
 * export class RemoteSyncFlag extends makeKVState<RemoteSyncFlag>()(
 *   'remote_sync_flag',
 *   Schema.Struct({
 ...
 * ```
 */
export const makeState = <T = unknown>(options?: MakeEventLogStateOptions | undefined) => {
  const spanPrefix = options?.spanPrefix ?? "EventLogState"
  const tableName = options?.tableName ?? EventLogStateTable

  return <K extends string, A, I>(name: K, schema: Schema.Schema<A, I>) => {
    const spanId = `${spanPrefix}.${name}`

    return makeContextService<T>()(
      name,
      Effect.fnUntraced(function* () {
        ...
        const findOne = SqlSchema.findOne({
          ...
          execute: (requestName) =>
            sql`SELECT json FROM ${sql(tableName)} WHERE name = ${requestName}`.pipe(
        ...
        const upsert = Effect.fn(`${spanId}.upsert`)(function* (input: A) {
          const data = yield* encode(input).pipe(Effect.orDie)
          yield* reactivity
            .mutation(
              reactivityKeys,
              sql`
                INSERT INTO ${sql(tableName)} ${sql.insert({ name, json: data })}
        ...
        const delete_ = reactivity
          .mutation(reactivityKeys, sql`DELETE FROM ${sql(tableName)} WHERE name = ${name}`.pipe(Effect.ignore))
          .pipe(Effect.withSpan(`${spanId}.delete`))

        const state: EventLogState<K, A> = {
          key: name,
          get,
          stream,
          upsert,
...
export class LocalSyncStats extends makeState<LocalSyncStats>()(
  ...
  static defaultValues: LocalSyncStats.Value = {
    usedStorageSize: 0
  }
}
export declare namespace LocalSyncStats {
  export type Value = EventLogState.Value<LocalSyncStats>
}
...
export declare namespace LocalSyncEvent {
  export type Value = EventLogState.Value<LocalSyncEvent>
}
...
export declare namespace RemoteSyncFlag {
  export type Value = EventLogState.Value<RemoteSyncFlag>
}
...
export declare namespace RemoteSyncStats {
  export type Value = EventLogState.Value<RemoteSyncStats>
}
...
export class SocketStatus extends makeState<SocketStatus>()("socket_status", SocketStateSchema) {
  ...
  setError(error: string, code?: number | undefined) {
    return this.upsert(
      SocketError.make({
        timestamp: new Date(),
        error,
...
export declare namespace SocketStatus {
  export type Value = EventLogState.Value<SocketStatus>
}
...
export declare namespace DevicesStatus {
  export type Value = EventLogState.Value<DevicesStatus>
}
...
const makeEventLogStates = Effect.gen(function* () {
  const sql = yield* SqlClient.SqlClient
...
export class EventLogStates extends Context.Tag("EventLogStates")<
  EventLogStates,
  Effect.Effect.Success<typeof makeEventLogStates>
>() {}

export const EventLogStatesLive = Layer.scoped(EventLogStates, makeEventLogStates)
