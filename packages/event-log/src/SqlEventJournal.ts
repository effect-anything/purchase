...
export const make = (options?: {
  ...
  Effect.gen(function* () {
    ...
    return EventJournal.EventJournal.of({
      ...
      writeFromRemote: Effect.fn("SqlEventJournal.writeFromRemote")(
        function* (remoteOptions) {
          const entries: Array<EventJournal.Entry> = []
          const remotes: Array<typeof RemoteSql.Type> = []
          for (const remoteEntry of remoteOptions.entries) {
            entries.push(remoteEntry.entry)
            remotes.push({
              remote_id: remoteOptions.remoteId,
          ...
          const uncommited = remoteOptions.entries.filter((entry_) => !existingIds.has(entry_.entry.idString))
          const brackets = remoteOptions.compact
            ? yield* remoteOptions.compact(uncommited)
          ...
          for (const [compacted] of brackets) {
            for (let i = 0; i < compacted.length; i++) {
              ...
              yield* remoteOptions.effect({ entry, conflicts })
            }
          }
        },
        Effect.mapError(
