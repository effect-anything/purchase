import type { FormatInternalError } from "@effect-x/react-router/errors/common"
import type { ReactRouterData, TypedResponse } from "@effect-x/react-router/response"
import type * as Vitest from "@effect/vitest"

import { detectLanguage } from "@effect-x/i18n/server"
...
import { expect as serverExpect, Testing } from "./test.ts"
...
function request<A, E>(loader: Action<A, E>, pathname_?: any | undefined, options_?: any | undefined) {
  ...
  const httpRequest = new Request(`http://localhost${pathname}`, {
  ...
  const handle = Effect.gen(function* () {
    ...
    const i18n = yield* Effect.promise(() => testInitI18n(httpRequest, appLoadContext.env))
    const requestLoadContext = {
      ...appLoadContext,
      i18n
    }
    ...
    if (maybeEffectLoader?.effect) {
      const handle = maybeEffectLoader.effect({
        ...
        context: requestLoadContext
      })

      return yield* handle.pipe(resToData(httpRequest))
    }
...
function encodeViaTurboStream(data: any, requestSignal: AbortSignal, streamTimeout: number | undefined) {
  ...
  } as TurboStream.EncodeOptions & { postPlugins: Array<(value: unknown) => [string, ...Array<unknown>] | undefined> })
}
...
const testInitI18n = (request: Request, _env: Record<string, any>) => {
  if (typeof globalThis.initI18n === "function") {
    return globalThis.initI18n(request)
  }
  ...
  return instance.init(initOptions).then(() => instance)
}
