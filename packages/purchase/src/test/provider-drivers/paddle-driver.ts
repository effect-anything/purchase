...
// const paddleBaseUrl = (environment: PublicPaddleScenarioConfig["paddleEnvironment"]) =>
...
export const makePaddleTestDriver = (input: {
  ...
  provider: "paddle",
  completeCheckout: (args) => completePaddleCheckout({ ...args, browser: input.browser }),
...
const completePaddleCheckout = Effect.fn(
  function* (
    input: CompleteProviderCheckoutInput & {
      readonly browser?: Required<PaymentTestBrowserOptions> | undefined
    }
  ) {
    ...
    yield* page.goto(input.checkoutUrl, { waitUntil: "domcontentloaded", timeout: 60_000 })
    ...
    if ((yield* country.count) > 0) {
      const countryCode = input.customer?.country ?? "US"
    ...
    if ((yield* postcode.count) > 0) {
      yield* postcode.fill(input.customer?.postcode ?? "10001")
    }
    ...
    yield* cardNumber.fill(input.paymentMethod?.cardNumber ?? "4242424242424242")
    yield* paymentFrame
      .locator('input[name="cardHolder"]')
      .fill(input.paymentMethod?.cardholderName ?? input.customer?.name ?? "Purchase SDK E2E User")
    yield* paymentFrame.locator('input[name="expiry"]').fill(input.paymentMethod?.expiry ?? "12/30")
    yield* paymentFrame.locator('input[name="cvv"]').fill(input.paymentMethod?.cvv ?? "100")
