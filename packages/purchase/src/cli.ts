...
export interface CommercialCatalogSyncPlan {
  ...
  readonly pricesToCreate: ReadonlyArray<{
    readonly offerId: string
    readonly providerOfferId: string
    readonly reason: string
    readonly ownership: string
  }>
  readonly localRowsToInsert: ReadonlyArray<{ readonly offerId: string; readonly reason: string }>
  readonly localRowsToUpdate: ReadonlyArray<{ readonly offerId: string; readonly reason: string }>
  readonly providerRefsToInsert: ReadonlyArray<{
    readonly ownerType: string
    readonly ownerId: string
    readonly providerId: string
  }>
  readonly providerRefsToUpdate: ReadonlyArray<{
    readonly ownerType: string
    readonly ownerId: string
    readonly providerId: string
  }>
  readonly staleRows: ReadonlyArray<{ readonly offerId: string; readonly reason: string }>
  readonly archiveCandidates: ReadonlyArray<{
    readonly ownerType: string
    readonly ownerId: string
    readonly providerId: string
...
const loadCatalogModule = async (options: CliOptions): Promise<CatalogModule> => {
  ...
  if (!selected) {
    throw new Error(`No catalog export found in ${options.modulePath}. Export plans/products or a BaseSDK subclass.`)
  }
  ...
  if (!plans || !products) {
    throw new Error(`Catalog module ${options.modulePath} must export both plans and products.`)
  }
...
const runCatalogSync = (
  ...
  Effect.gen(function* () {
    ...
    const sdk = yield* catalog.tag
    return yield* sdk.catalog.sync({ dryRun: options.dryRun })
  }).pipe(
    Effect.provide(catalog.layer),
...
const catalogSync = Command.make("sync", catalogSyncOptions, (config) =>
  Effect.tryPromise({
    try: async () => {
      const options = parseCatalogSyncOptions(config)
      const catalog = await loadCatalogModule(options)
      return {
        options,
...
const catalog = Command.make("catalog").pipe(
  Command.withSubcommands([catalogSync]),
  Command.withDescription("Catalog sync commands.")
)

const root = Command.make("pay").pipe(Command.withSubcommands([catalog]))
