  ...
  it.effect("catalog.sync dry run does not call provider or write local rows", () => {
    ...
    return runPayEffect(
      Effect.gen(function* () {
        ...
        expect(result.plan.productsToCreate.map((entry) => entry.productId).toSorted()).toEqual([
          "ai_credit_pack",
          "lifetime_product",
          "saas"
        ])
        expect(result.plan.pricesToCreate.map((entry) => entry.offerId).toSorted()).toEqual([
          testOfferIds.credits100,
          testOfferIds.lifetime,
          testOfferIds.free,
          testOfferIds.proMonthly
        ])
        expect(result.plan.localRowsToInsert.map((entry) => entry.offerId).toSorted()).toEqual([
          testOfferIds.credits100,
          testOfferIds.lifetime,
          testOfferIds.free,
          testOfferIds.proMonthly
