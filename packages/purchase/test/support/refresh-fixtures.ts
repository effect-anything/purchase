...
import { ensureGeneratedFixtureDir, generatedFixturePath, type StoredWebhookFixture } from "./generated-fixture.ts"
import {
  capturePaddleScenarioWebhook,
  capturePaddleSimulation,
  ensurePaddleNotificationSetting
} from "./paddle-simulator.ts"
import { paddleWebhookEvents, stripeWebhookEvents } from "./provider-events.ts"
import { captureStripeSubscriptionResumedWebhook, captureStripeWebhook } from "./stripe-cli.ts"

const cliArgs = process.argv.slice(2).filter((value) => value !== "--")
const args = new Set(cliArgs)
const providers: ReadonlyArray<PaymentProviderTag> = args.has("--stripe")
  ? ["stripe"]
  : args.has("--paddle")
    ? ["paddle"]
    : ["stripe", "paddle"]
...
const writeFixture = (name: PaymentProviderTag, eventType: string, fixture: StoredWebhookFixture) =>
  Effect.sync(() => {
