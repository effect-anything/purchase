...
export const mock =
  ...
  <A, E, R>(layer: Layer.Layer<A, E, R>) =>
    layer.pipe(Layer.provide(makeTestLayer(tag)(deepMerge(service2, service1) as Partial<S>)))
...
export const withLayer =
  ...
  <A, E, R>(effect: Effect.Effect<A, E, R>) =>
    effect.pipe(Effect.provide(makeTestLayer(tag)(deepMerge(service2, service1) as Partial<S>)))
...
const defaultApi = Object.assign(V.it, { scopedFixtures: V.it.scoped }) as unknown as Vitest.API
...
const runPromise =
  <R1>(run: RunPromise, ctx?: TestContext | undefined) =>
  <A, E, R extends R1>(effect: Effect.Effect<A, E, R>) =>
    Effect.gen(function* () {
      const exitFiber = yield* Effect.fork(Effect.exit(effect))

      V.onTestFinished(async () => {
        Fiber.interrupt(exitFiber).pipe(Effect.asVoid, Effect.runPromise)
      })

      const exit = yield* Fiber.join(exitFiber).pipe(Effect.timeout(Duration.millis(ctx?.task.timeout ?? 15_000)))
      ...
      .pipe((scopedEffect) => run(scopedEffect, { signal: ctx?.signal }))
      .then((f) => f())
  ...
  <R>(mapEffect: <A>(self: Effect.Effect<A, any, R>) => Effect.Effect<A, any, R>, it: Vitest.API = defaultApi) =>
    (run_: RunPromise): Vitest.Vitest.Tester<R> => {
      const run = <A, E, TestArgs extends Array<unknown>>(
        ...
        pipe(
          Effect.suspend(() => {
            const testCtx = args[0] as V.TestContext

            if (testCtx) {
              return Effect.withSpan(self(...args), `Test-${testCtx.task.suite?.name}-${testCtx.task.name}`, {
                attributes: {
                  filename: testCtx.task.file.name
                }
              })
            }
      ...
      const f: Vitest.Vitest.Test<R> = (name, self, timeout) =>
        it(name, testOptions(timeout) as any, (ctx: V.TestContext) => run(ctx, [ctx], self))

      const skip: Vitest.Vitest.Tester<R>["only"] = (name, self, timeout) =>
        // @ts-ignore
        it.skip(name, testOptions(timeout), (ctx: V.TestContext) => run(ctx, [ctx], self))

      const skipIf: Vitest.Vitest.Tester<R>["skipIf"] = (condition) => (name, self, timeout) =>
        // @ts-ignore
        it.skipIf(condition)(name, testOptions(timeout), (ctx: V.TestContext) => run(ctx, [ctx], self))

      const runIf: Vitest.Vitest.Tester<R>["runIf"] = (condition) => (name, self, timeout) =>
        // @ts-ignore
        it.runIf(condition)(name, testOptions(timeout), (ctx: V.TestContext) => run(ctx, [ctx], self))

      const only: Vitest.Vitest.Tester<R>["only"] = (name, self, timeout) =>
        // @ts-ignore
        it.only(name, testOptions(timeout), (ctx: V.TestContext) => run(ctx, [ctx], self))

      const each: Vitest.Vitest.Tester<R>["each"] = (cases) => (name, self, timeout) =>
        it.for(cases)(name, testOptions(timeout), (args, ctx: V.TestContext) => run(ctx, [args] as any, self as any))

      const fails: Vitest.Vitest.Tester<R>["fails"] = (name, self, timeout) =>
        it.fails(name, testOptions(timeout), (ctx: V.TestContext) => run(ctx, [ctx], self))
      ...
      const prop: Vitest.Vitest.Tester<R>["prop"] = (name, arbitraries, self, timeout) => {
        if (Array.isArray(arbitraries)) {
          ...
          return it(name, testOptions(timeout) as any, (ctx: V.TestContext) =>
            // @ts-ignore
            FC.assert(
              // @ts-ignore
...
export const layer =
  ...
  (
    ...
    const makeIt = (it: Vitest.API): Vitest.Vitest.MethodsNonLive<R, ExcludeTestServices> =>
      Object.assign(it, {
        effect: makeTester<TestServices.TestServices | Scope.Scope | R>(
          (effect) =>
            options.hooks?.mapEffect
              ? options.hooks.mapEffect(Effect.scoped(effect), mangedRuntime)
              : Effect.locally(
    ...
    const setup = () => {
      ...
      V.beforeAll(async () => await mangedRuntime.runtime(), timeout)
      ...
      V.afterAll(async () => await mangedRuntime.dispose(), timeout)
    }
...
function deepMerge<T extends object, U extends object>(base: T, override: U): T & U {
