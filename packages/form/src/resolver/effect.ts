...
export function effectTsResolver<Input extends FieldValues, Context, Output>(
  ...
  return (values, _, options) => {
    const effect = decodeUnknown(
      schema,
      schemaOptions
    )(values).pipe(
      Effect.catchAll((parseIssue) => Effect.flip(ArrayFormatter.formatIssue(parseIssue))),
        ...
        const errors = issues.reduce(
          (acc, error) => {
            ...
            if (validateAllFieldCriteria) {
              ...
              const messages = types?.[String(error._tag)]
              ...
              acc[key] = appendErrors(
                ...
                acc,
                error._tag,
                messages ? ([] as Array<string>).concat(messages as Array<string>, error.message) : error.message
              ) as FieldError
            }
      ...
      Effect.tap(() => Effect.sync(() => options.shouldUseNativeValidation && validateFieldsNatively({}, options))),
      Effect.match({
    ...
    return resolverOptions.mode === "sync" ? Effect.runSync(effect) : Effect.runPromise(effect)
  }
}
