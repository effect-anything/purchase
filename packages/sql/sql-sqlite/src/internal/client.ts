...
import { type QueryResult, SqliteQueryExecute, SqliteQueryStreamExecute } from "../schema.ts"

/**
 * @category type ids
 * @since 1.0.0
 */
export const TypeId: unique symbol = Symbol.for("@effect-x/sql-sqlite/sqlite-client")
 ...
 * @since 1.0.0
 */
export const SqlClient = Context.GenericTag<SqlClient>("@effect-x/sql-sqlite/sqlite-client")
  ...
  Effect.gen(function* () {
    ...
    const makeConnection = Effect.gen(function* () {
      return identity<SqliteConnection>({
        execute(sql, params, transformRows_) {
          return transformRows_ ? Effect.map(run(sql, params), transformRows_) : run(sql, params)
        },
        ...
        executeUnprepared(sql, params, transformRows_) {
          return this.execute(sql, params, transformRows_)
        },
        executeStream(sql, params, transformRows_) {
          return pipe(
            relay.runStream(new SqliteQueryStreamExecute({ sql, params: params as any }, { disableValidation: true })),
            transformRows_
              ? Stream.mapChunks((chunk) => Chunk.unsafeFromArray(transformRows_(Chunk.toReadonlyArray(chunk))))
              : identity,
