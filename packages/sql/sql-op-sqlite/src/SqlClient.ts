 ...
 * @since 1.0.0
 */
export const TypeId: unique symbol = Symbol.for("@effect-x/sql-op-sqlite/SqliteClient")
 ...
 * @since 1.0.0
 */
export const SqliteClient = Context.GenericTag<SqliteClient>("@effect-x/sql-op-sqlite/sqlite-client")
 ...
 * @since 1.0.0
 */
export const asyncQuery: FiberRef.FiberRef<boolean> = globalValue("@effect-x/sql-op-sqlite/Client/asyncQuery", () =>
        ...
        Effect.withFiberRuntime<Array<any>, SqlError>((fiber) => {
          if (fiber.getFiberRef(asyncQuery)) {
            return Effect.map(
              ...
              (result) => (result as { readonly rows?: ReadonlyArray<any> }).rows ?? []
            )
          }
          return Effect.try({
