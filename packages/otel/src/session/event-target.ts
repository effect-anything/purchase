...
export class InternalEventTarget {
  protected events: Partial<{ [T in keyof OtelWebEventTypes]: Array<EventListener<T>> }> = {}

  addEventListener<T extends keyof OtelWebEventTypes>(type: T, listener: EventListener<T>): void {
    if (!this.events[type]) {
      this.events[type] = []
    }
    ;(this.events[type] as Array<EventListener<T>>).push(listener)
  }
  ...
  removeEventListener<T extends keyof OtelWebEventTypes>(type: T, listener: EventListener<T>): void {
    ...
    const i = (this.events[type] as Array<EventListener<T>>).indexOf(listener)
