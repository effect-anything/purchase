...
```mermaid
flowchart LR
  SDK["Catalog and SDK Tag"] --> Runtime["Managed Runtime"]
  Provider["Provider Layer"] --> Runtime
  SQL["SQL Client Layer"] --> Runtime
  Runtime --> Out["Checkout<br/>Webhooks<br/>Customer Reads"]
```
