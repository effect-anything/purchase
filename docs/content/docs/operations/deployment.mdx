...
```mermaid
flowchart LR
  App["App API"] --> Runtime["Purchase Runtime"]
  Webhook["Webhook Endpoint"] --> Runtime
  Runtime --> DB[("SQL Database")]
  Runtime --> Provider["Stripe or Paddle"]
  CLI["Catalog Sync CLI"] --> Runtime
```
