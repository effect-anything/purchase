...
```mermaid
flowchart LR
  DSL["Catalog DSL"] --> Catalog["Commercial Catalog"]
  Catalog --> WF["Checkout and Mutation Workflows"]
  WF --> Adapter["Provider Adapter"]
  Adapter --> Webhook["Webhook Events"]
  Webhook --> Normalized["Normalized Commercial Events"]
  Normalized --> Projection["Projection Refresh"]
  Projection --> Snapshot["Customer Snapshot<br/>Entitlements<br/>Credits Wallet"]
  Snapshot --> App["Application Product Logic"]
```
