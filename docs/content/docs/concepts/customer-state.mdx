...
```mermaid
flowchart LR
  WF["Checkout or Webhook"] --> Store["Workflow Store"]
  Store --> Events["Commercial Events"]
  Events --> Projection["Projection Refresh"]
  Projection --> Snapshot["Customer Snapshot"]
  Projection --> Ent["Entitlement Snapshot"]
  Projection --> Wallet["Credits Wallet"]
```
